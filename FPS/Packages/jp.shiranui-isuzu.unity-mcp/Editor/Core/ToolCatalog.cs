    ...
    internal sealed class ToolCatalog
    {
        ...
        private static bool IsTestFixtureType(Type type)
        {
            for (var current = type; current != null; current = current.DeclaringType)
            {
                ...
                try
                {
                    attributes = current.GetCustomAttributes(false);
                }
                catch (Exception)
                {
                    // A plugin that bundles its own renamed copy of a compiler assembly carries
                    // attributes whose types cannot be resolved in this AppDomain. One such
                    // type must not abort the whole catalog scan.
                    continue;
                }
            ...
            Func<ToolCatalog, List<string>, IReadOnlyList<McpToolDescriptor>> defined)
        {
            ...
            foreach (var type in types)
            {
                ...
                foreach (var method in type.GetMethods(flags))
                {
                    ...
                    {
                        // Same unresolvable-attribute case as IsTestFixtureType; the type is
                        // foreign to this package and cannot declare an [McpTool] anyway.
                        continue;
                    }
